...
export function Button({
  ...
  if (href) {
    return (
      <Link href={href} className={computedClassName} {...(props as React.AnchorHTMLAttributes<HTMLAnchorElement>)}>
        {icon && <span>{icon}</span>}
        {children}
      </Link>
    );
  ...
  return (
    <button className={computedClassName} disabled={disabled} {...(props as React.ButtonHTMLAttributes<HTMLButtonElement>)}>
      {icon && <span>{icon}</span>}
      {children}
    </button>
  );
